...
{foreach from=$_items item=item}
    <li>{if time() - 600 <= strtotime($item->date)}<span class="newmark">NEW</span>{/if}
        ...
        {assign var=twitter_message value=$item->body|mbtruncate:100:"…"|cat:"\n#RSSでTODO管理 →`$config.site_top`"}
        <a href="https://twitter.com/intent/tweet?text={$twitter_message|escape:"url"}" target="_blank">Twitterに書く</a>
    </li>
{/foreach}
</ul>
